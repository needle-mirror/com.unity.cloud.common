...
namespace Unity.Cloud.Common.Editor
{
    ...
    {
        [SerializeField] public IAppInfoProvider m_AppInfoProvider;
        [SerializeField] IHttpClient m_HttpClient;
        [SerializeField] IServiceHttpClient m_ServiceHttpClient;
        [SerializeField] IServiceHostResolver m_ServiceHostResolver;
        ...
        [SerializeField] IAuthenticator m_UnityEditorAuthenticator;

        List<AppInfo> m_Applications = new List<AppInfo>();
        List<CloudOrganization> m_Organizations = new List<CloudOrganization>();
        ...
        internal const string k_organizationUri = "https://services.unity.com/api/unity/v1/users/me/organizations";
        ...
        {
            public CloudOrganization[] organizations;
        }
        ...
        public delegate void SelectAppDelegate(string appId, string appName, string displayName);
...
#if USE_UC_IDENTITY
            var targetClientIdTokenToUnityServicesTokenExchanger = new TargetClientIdTokenToUnityServicesTokenExchanger(m_HttpClient, m_ServiceHostResolver);
            m_UnityEditorAuthenticator = new UnityEditorAuthenticator(targetClientIdTokenToUnityServicesTokenExchanger);
            
            m_ServiceHttpClient = new ServiceHttpClient(m_HttpClient, m_UnityEditorAuthenticator, UnityCloudPlayerSettings.Instance);
            m_AppInfoProvider = new AppInfoProvider(m_ServiceHttpClient, m_ServiceHostResolver);

            m_UnityEditorAuthenticator.AuthenticationStateChanged += UnityEditorAuthenticator_AuthenticationStateChanged;
            await m_UnityEditorAuthenticator.InitializeAsync().ConfigureAwait(true);
#else
            m_ServiceHttpClient = new ServiceHttpClient(m_HttpClient, null, UnityCloudPlayerSettings.Instance);
            m_AppInfoProvider = new AppInfoProvider(m_ServiceHttpClient, m_ServiceHostResolver);
#endif
        }
        ...
        public void DrawGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("Please login to access existing apps or to register a new app. If you are unable to login, try restarting the editor.", MessageType.Warning);
            }
#else
            ...
            if (GUILayout.Button("com.unity.cloud.identity Package Documentation"))
            {
                ...
                {
                    Application.OpenURL($"{k_DocumentationUriScheme}{k_DocumentationLastestUrl}");
                }, null);
            }
#endif
            GUILayout.Space(10);
        ...
        {
            var newSelectedOrganizationIndex = EditorGUILayout.Popup("Organization", m_SelectedOrganizationIndex, m_Organizations.Select(item => item.name).ToArray());
        ...
        void RegisterNewAppUI()
        {
            ...
            EditorGUILayout.HelpBox("App name should be globally unique, alphanumeric, lowercase, between 4-10 characters", MessageType.Info);
        ...
        void ShowExistingAppsUI()
        {
            ...
            foreach (var app in m_Applications)
            {
                ...
                GenerateField("App ID", app.Id);

                EditorGUILayout.EndVertical();

                if (GUILayout.Button("Select", GUILayout.Width(80), GUILayout.Height(60)))
                {
                    m_SelectAppDelegate(app.Id, app.Name, app.DisplayName);
                }
        ...
        static void GenerateField(string fieldLabel, string fieldValue, float fieldLabelWidth = 200, bool showCopyButton = true)
        {
            EditorGUILayout.BeginHorizontal();
        ...
        async Task GetApplications()
        {
            try
            {
                m_Applications = await m_AppInfoProvider.GetAppsInfoAsync(new OrganizationId(m_Organizations[m_SelectedOrganizationIndex].genesisId));
        ...
        async Task<bool> GetOrganizations()
        {
            try
            {
                var response = await m_ServiceHttpClient.GetJsonAsync<OrganizationsList>(k_organizationUri);

                m_Organizations.Clear();
                m_Organizations = new List<CloudOrganization>(response.organizations);
        ...
        async Task RegisterNewApp()
        {
            try
            {
                var registerUri = m_ServiceHostResolver.GetResolvedRequestUri("/api/applications");
                var registerPayload = new AppRegistrationPayload
                {
                    Name = m_NewAppName,
                    OrgId = m_Organizations[m_SelectedOrganizationIndex].genesisId,
                ...
                m_SelectAppDelegate(newAppInfo.Id, newAppInfo.Name, newAppInfo.DisplayName);
        ...
        void EditApp(string appId, string appName, string displayName)
        {
            var editWindow = CreateInstance<EditApplicationPopup>();
            editWindow.Initialize(this, appName, displayName, appId, m_Organizations[m_SelectedOrganizationIndex].genesisId);
        ...
        void DeleteApp(string appId, string appName, string displayName)
        {
            var deleteWindow = CreateInstance<DeleteApplicationPopup>();
            deleteWindow.Initialize(this, appName, displayName, appId, m_Organizations[m_SelectedOrganizationIndex].genesisId);
            var cursorPosition = GUIUtility.GUIToScreenPoint(Event.current.mousePosition);
        ...
        class EditApplicationPopup : EditorWindow
        {
            ...
            public void Initialize(UnityCloudAppRegistration appRegistration, string originalAppName, string originalAppDisplayName, string appId, string orgId)
            {
            ...
            async Task EditApplication()
            {
                try
                {
                    var registerUri = m_AppRegistration.m_ServiceHostResolver.GetResolvedRequestUri($"/api/applications/{m_AppId}");
                    ...
                    var httpContent = new StringContent(JsonSerialization.Serialize(updatePayload), Encoding.UTF8, "application/json"); 
                    ...
                    if (updatedAppInfo.Id == null || updatedAppInfo.Name == null)
                    {
                        ...
                        EditorUtility.DisplayDialog("App Registration Error", $"An error occured when editing the application: {errorMessage.Message}", "Ok", "");
        ...
        class DeleteApplicationPopup : EditorWindow
        {
            ...
            public void Initialize(UnityCloudAppRegistration appRegistration, string appName, string appDisplayName, string appId, string orgId)
            {
                m_AppRegistration = appRegistration;
                m_AppId = appId;
                m_OrgId = orgId;
            ...
            void OnGUI()
            {
                ...
                GenerateField("Application ID", m_AppId, showCopyButton:false);
                
                EditorGUILayout.Space(10);

                GUILayout.Label("Are you sure you want to delete this application? Once deleted, the application can not be recovered.", EditorStyles.wordWrappedLabel);
            ...
            async Task DeleteApplication()
            {
                try
                {
                    ...
                    var deleteUri = m_AppRegistration.m_ServiceHostResolver.GetResolvedRequestUri($"/api/applications/{m_AppId}?orgId={m_OrgId}");
                    var response = await m_AppRegistration.m_ServiceHttpClient.DeleteAsync(deleteUri);
                ...
                {
                    EditorUtility.DisplayDialog("App Registration Error", $"An error occured when deleting the application: {ex.Message}", "Ok", "");
                    Debug.LogException(ex);
                }
            }
        }
