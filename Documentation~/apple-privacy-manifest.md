...
Unity Cloud Common SDK does not collect data or engage in any data practices requiring disclosure in a privacy manifest file. It only provides tools for other leaf packages to do so. Dev-users shouldn't install Common directly, therefore they should refer to the leaf packages that they're willing to use for the applicable SDKs.

> [!NOTE]
> Note: The Unity Cloud Common package is dependent on the following services. Refer to their manifest files for applicable data practices.
>
